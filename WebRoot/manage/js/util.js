 ...
 * @returns
 */
function getCurrtRowData(datagrid, index) {
 ...
 * 2015-12-17 1.0
 * 
 * params:form return Object
 * 
 * ps:form为表单对象(jquery类型或节点)
 * 
 ...
 * url:haveWebName==true?u:(getWebProjectName()+u), dataType:dt, data:da,
 ...
 * @param parentId
 */
function firstOption(parentId) {
	var selects = $("#" + parentId + " select");
	for (var i = 0; i < selects.length; i++) {
		$(selects[i]).find("option:first").prop("selected", "selected");
	}
}
/**
 * 将selects的值整合，以/分割
 */
function groupSelect(selects) {
	var address = "";
	for (var index = 0; index < selects.length; index++) {
		var val = $(selects[index]).val();
